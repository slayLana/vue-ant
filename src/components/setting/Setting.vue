...
import ColorCheckbox, {
  ColorCheckboxGroup
} from "../checkbox/ColorCheckbox.vue";
import ImgCheckbox, { ImgCheckboxGroup } from "../checkbox/ImgCheckbox.vue";
import Clipboard from "clipboard";
// import ImgCheckboxGroup from "../checkbox/ImgCheckboxGroup.vue";
// import ColorCheckboxGroup from "../checkbox/ColorCheckboxGroup.vue";

@Component({
  name: "Setting",
  components: {
    [ImgCheckboxGroup.name]: ImgCheckboxGroup,
    [ImgCheckbox.name]: ImgCheckbox,
    [ColorCheckboxGroup.name]: ColorCheckboxGroup,
    [ColorCheckbox.name]: ColorCheckbox,
    [StyleItem.name]: StyleItem,
...
export default class Setting extends Vue {
  ...
  protected onColorChange(values: string[], colors: string[]) {
    if (colors.length > 0) {
      this.$message.info(`您选择了主题色 ${colors}`);
    }
  }

  protected setTheme(values: any) {
    this.$store.commit("setting/setTheme", values[0]);
  }

  protected setLayout(values: any) {
  ...
  protected setMultipage(checked: any) {
