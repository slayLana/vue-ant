<template>
  <form :form="form">
    ...
    <a-button style="width: 100%; margin-top: 16px; margin-bottom: 8px"
              type="dashed"
              icon="plus"
              @click="newMeber">新增成员</a-button>
  </form>
</template>

<script lang='ts'>
import { Vue, Component } from "vue-property-decorator";
import { Table, Input, Divider, Popconfirm, Button } from "ant-design-vue";
const columns = [
  {
    title: "成员姓名",
    dataIndex: "name",
...
export default class TableForm extends Vue {
  ...
  private remove(key: any) {
    const newData = this.dataSource.filter((item: any) => item.key !== key);
    this.dataSource = newData;
  }
  private saveRow(key: any) {
    const target = this.dataSource.filter((item: any) => item.key === key)[0];
    target.editable = false;
    target.isNew = false;
  }
  private toggle(key: any) {
    const target = this.dataSource.filter((item: any) => item.key === key)[0];
